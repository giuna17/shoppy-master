...
export const RecentlyViewed = ({
  ...
  const getLocalizedField = useCallback(
    (field: any, fieldType: 'name' | 'description' = 'name'): string => {
  ...
  const fetchRecentlyViewed = useCallback(async () => {
    ...
    try {
      ...
      const uniqueItems = items.reduce<{ [key: number]: any }>((acc, item) => {
        if (
          !acc[item.productId] ||
          (item.timestamp || 0) > (acc[item.productId].timestamp || 0)
        ) {
          acc[item.productId] = item;
        } else {
          // Update view count for existing items
          acc[item.productId].viewCount =
            (acc[item.productId].viewCount || 0) + 1;
        }
        return acc;
      }, {});
      ...
      console.log(
        `Processed ${items.length} items, ${sortedUniqueItems.length} unique after deduplication`,
      );
      ...
      const validProducts = (await Promise.all(productPromises)).filter(
        (p): p is RecentlyViewedProduct => p !== null,
      );
      ...
      console.log(
        'Updated recentlyViewed state with',
  ...
  const handleToggleFavorite = async (
    ...
    try {
      ...
      setRecentlyViewed((prev) =>
        prev.map((p) =>
          p.id === product.id ? { ...p, isFavorite: newFavoriteStatus } : p,
        ),
      );
      ...
      await recentlyViewedService.updateFavoriteStatus(
        product.id,
        newFavoriteStatus,
      );
      ...
      setRecentlyViewed((prev) =>
        prev.map((p) =>
          p.id === product.id ? { ...p, isFavorite: !product.isFavorite } : p,
        ),
      );
    }
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="relative">
          ...
          {showLeftButton && (
            <button
              onClick={() => scroll('left')}
              className="absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-background/80 backdrop-blur-sm p-2 rounded-full shadow-lg hover:bg-foreground/5 transition-colors"
          ...
          {showRightButton && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 z-20 bg-background/80 backdrop-blur-sm p-2 rounded-full shadow-lg hover:bg-foreground/5 transition-colors"
          ...
          <div
            ...
            className="flex overflow-x-auto pb-4 -mx-2 px-2 scrollbar-hide snap-x"
